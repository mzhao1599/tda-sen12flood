...
def run_late_fusion_single(modality: str, stacked_dir: str, cubical_dir: str,
    ...
    train_dl = DataLoader(torch.utils.data.Subset(ds, train_idx), BATCH_SIZE, shuffle=True,  collate_fn=collate_fusion_single, num_workers=0, worker_init_fn=lambda wid: np.random.seed(DEFAULT_SEED+wid), generator=g)
    val_dl   = DataLoader(torch.utils.data.Subset(ds, val_idx),   BATCH_SIZE, shuffle=False, collate_fn=collate_fusion_single, num_workers=0, worker_init_fn=lambda wid: np.random.seed(DEFAULT_SEED+wid), generator=g)
...
def run_late_fusion_dual(s1_stacked_dir: str, s2_stacked_dir: str,
    ...
    g = torch.Generator(); g.manual_seed(DEFAULT_SEED)
    train_dl = DataLoader(torch.utils.data.Subset(ds, train_idx), BATCH_SIZE, shuffle=True,  collate_fn=collate_fusion_dual, num_workers=0, worker_init_fn=lambda wid: np.random.seed(DEFAULT_SEED+wid), generator=g)
    val_dl   = DataLoader(torch.utils.data.Subset(ds, val_idx),   BATCH_SIZE, shuffle=False, collate_fn=collate_fusion_dual, num_workers=0, worker_init_fn=lambda wid: np.random.seed(DEFAULT_SEED+wid), generator=g)
