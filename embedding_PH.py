...
def run_training(s1_dir: str, s2_dir: str, modality: str='s1', epochs: int=NUM_EPOCHS, seed: int=DEFAULT_SEED, bidirectional: bool=True):
    print(f"\n=== {modality.upper()} Training (Quartile-qpersemb) ==="); checkpoint_dir='.'; print(f"  Checkpoint dir: {checkpoint_dir}")
    ...
    best_score=0.0; best_epoch=0; ckpt_name='dual_quartile_qpersemb_gru_best.pt' if is_dual else f'quartile_qpersemb_gru_{modality}.pt'; patience=20
    ...
    save_detailed_predictions_to_file(sweep_results, '.', modality)
...
def parse_args():
    p=argparse.ArgumentParser(description='Quartile-qpersemb GRU flood sequence classifier')
...
def main():
    ...
    log_file_path='quartile_qpersemb_gru.txt'; log_fh=open(log_file_path,'a', buffering=1); atexit.register(log_fh.close); globals()['log_file']=log_fh; sys.stdout=Tee()
    ...
    if s1_exists and s2_exists:
        print('\n=== FULL QUARTILE-qpersemb PIPELINE (S1+S2) ===')
        try:
